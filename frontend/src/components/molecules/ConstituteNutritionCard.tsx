...
const ConstituteNutritionCard: React.FC<ConstituteNutritionCardProps> = ({
  ...
  const [dishNutrition, setDishNutrition] = useState<null | {
    calories: number;
    protein: number;
    carbs: number;
    fat: number;
  ...
  return (
    <Card>
      <div className="flex justify-between items-center">
          ...
          {constitute.food && constitute.food_quantity && (
            <div className="text-xs text-gray-500 space-y-1 mt-1">
              ...
              <div>
                ...
                {((constitute.food.protein * constitute.food_quantity) / 100).toFixed(2)} g
              </div>
              <div>
                <strong>Glucides:</strong>{' '}
                {((constitute.food.carbs * constitute.food_quantity) / 100).toFixed(2)} g
              </div>
              <div>
                <strong>Lipides:</strong>{' '}
                {((constitute.food.fat * constitute.food_quantity) / 100).toFixed(2)} g
              </div>
            </div>
          )}
            ...
            <div className="text-xs text-gray-500 space-y-1 mt-1">
              ...
              {dishNutrition && (
                <>
                  <div>
                    <strong>Calories:</strong>{' '}
                    {(dishNutrition.calories * constitute.dish_quantity).toFixed(0)} kcal
                  </div>
                  <div>
                    <strong>Protéines:</strong>{' '}
                    {(dishNutrition.protein * constitute.dish_quantity).toFixed(2)} g
                  </div>
                  <div>
                    <strong>Glucides:</strong>{' '}
                    {(dishNutrition.carbs * constitute.dish_quantity).toFixed(2)} g
                  </div>
                  <div>
                    <strong>Lipides:</strong>{' '}
                    {(dishNutrition.fat * constitute.dish_quantity).toFixed(2)} g
                  </div>
                </>
              )}
            </div>
        ...
        {onRemove && (
          <button onClick={onRemove} className="text-red-600 hover:text-red-800 text-sm ml-4">
            Supprimer
          </button>
        )}
      </div>
